...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        noise_scale = torch.sqrt(1 - current_alpha_t)
        noise_scale = torch.sqrt((1 - alpha_prod_t_prev) / (1 - alpha_prod_t) * current_beta_t)
        ...
        # Third order:
        if previous_output != None and two_previous_output != None:
            newScore = previous_output
            newSample = sample/(current_alpha_t ** (0.5)) + (torch.sqrt(1-alpha_prod_t_prev)-torch.sqrt((1-alpha_prod_t)/current_alpha_t))*model_output
            term2 = torch.sqrt(1/current_alpha_t)
            term3 = (sample-0.5*(1-current_alpha_t)*model_output/(beta_prod_t ** (0.5)))
            ...
            NewTerm1 = (1-4*current_alpha_t)*torch.arcsin(alpha_prod_t_prev**0.5)-(torch.sqrt(1-alpha_prod_t_prev)*(alpha_prod_t_prev+2*alpha_prod_t**2)/torch.sqrt(alpha_prod_t_prev))
            NewTerm2 = (1-4*current_alpha_t)*torch.arcsin(alpha_prod_t**0.5)-torch.sqrt(1-alpha_prod_t)*(alpha_prod_t+2*alpha_prod_t**2)/torch.sqrt(alpha_prod_t)
            NewTerm3 = 0.25*torch.sqrt(alpha_prod_t_prev)/((alpha_prod_t-alpha_prod_t_next)**2)
